...
## MADSC (Pattern Recognition 179, 2026) — NO PUBLIC CODE (checked 2026-08-13)
Searched the GitHub API for `MADSC`, `MADSC MABSA`, and
`aspect-aware description calibration MABSA`; every hit is an unrelated project
(MADSci lab automation, MadScienceLabDocker, madscience Ruby gem, …). The paper lists no
repository URL and states only "Data will be made available on request". Accepted
2026-04-10, online 2026-04-13, so a release may simply not exist yet.

**Consequence for this project:** MADSC (the current t2015 bar, JMASA 72.9) can be read but
not run or rescored — the same situation as VLHA, and unlike AoM/DQPSA/CORSA. Its
mechanisms were therefore reimplemented from the paper text as `experts/aadg.py` (AADG +
dual similarity) and `experts/masc_gated.py` (confidence calibrator + modality gate), with
every substitution recorded in `possible-patches.md` §C.5/§C.10 rather than copied.

## CORSA — cloned and readable, but `senticnet_word.txt` is dead weight
`CORSA/src/senticnet_word.txt` (39,891 word→polarity entries) ships with the repo but is
**never read by any Python file** in the release (verified by grep over all `*.py`). Nothing
to borrow there. CORSA's t2015 joint F1 (69.9) is already below this session's 70.29.
