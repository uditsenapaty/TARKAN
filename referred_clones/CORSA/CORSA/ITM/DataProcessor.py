...
class MyDataset(Data.Dataset):
    def __init__(self,data_dir,imagefeat_dir,tokenizer,max_seq_len=64,num_roi_boxes=100,img_feat_dim=2048):
    ...
    def transform(self,line,index ):
        ...
        value=line
        text_a = ' '.join(value['words'])

        input_ids=self.tokenizer(text_a.lower())['input_ids']   #  <s>text_a</s></s>text_b</s>
        ...
        #img_id=0
        a_img_id = value['image_id']

        img_feat=np.zeros((num_roi_boxes,self.img_feat_dim))

        a_img_feat=read_npz(self.imagefeat_dir,a_img_id)
...
def read_npz(imagefeat_dir,img_id):
    ...
    feat_dict=np.load(os.path.join(imagefeat_dir,img_id+'.npz'))
